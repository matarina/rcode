...
fold = [2,5,6,7,9]
files = glob.glob(f'/data/dk/ucec/CLAM/eval_results/EVAL_eval_code2/fold_{fold}.csv')
#files = glob.glob('/data/dk/ucec/CLAM/eval_results/EVAL_eval_code2/fold_10.csv')
fprs, tprs = [], []
i = 0
lb = LabelBinarizer()
for file in files:
# Parse the table pd.read_csv(file)
    ...
    y_pred_probs = pd.read_csv(file)[['p_0','p_2', 'p_3']].values
...
for i in [2,5,6,7,9]:
    file_name = f'/data/dk/ucec/CLAM/eval_results/EVAL_eval_code2/fold_{i}.csv'
...
data = pd.read_csv('/data/dk/ucec/CLAM/eval_results/EVAL_eval_code2/fold_2.csv')

Y_test = data['Y']
lb = LabelBinarizer()
yhot = lb.fit_transform(Y_test)
y_score = data[['p_0','p_2', 'p_3']].values
precision = dict()
recall = dict()
